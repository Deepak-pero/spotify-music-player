...
async function getsongs(folder) {
    currFolder = folder;
    let response = await fetch(`http://127.0.0.1:57992/${folder}`);
...
async function displayAlbums() {
    try {
        let response = await fetch(`http://127.0.0.1:57992/songs/`);
        let html = await response.text();
        let div = document.createElement("div");
        div.innerHTML = html;
        ...
        for (let index = 0; index < array.length; index++) {
            ...
            if (e.href.includes("/songs")) {
                ...
                let infoResponse = await fetch(`http://127.0.0.1:57992/${folder}/info.json`);
                if (infoResponse.ok) {
